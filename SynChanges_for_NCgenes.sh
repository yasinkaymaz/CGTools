...
then
  ...
  echo -e "#Gene\tNum_Tot_Vars\tSNV-Changes" > $outDir/NC.Genes.SNVs.txt
  ...
  do
    echo $geneOfvariant;
    grep -w $geneOfvariant $CDSbed > $outDir/"${geneOfvariant}".bed;
...
#    python $toolDir/MSA_distanceCalc.py MeandNdS -af $genemultifasta -rn $geneOfvariant >> pairwiseMeandNdS.txt
    ...
    list_Ss=`cat "${genemultifasta%.fasta}".fasta_SNV_positions.bed|cut -f4 |tr '\n' ','|sed 's/.$//'`

    echo -e "$geneOfvariant\t$SNVs\t$list_Ss" >> $outDir/NC.Genes.SNVs.txt
